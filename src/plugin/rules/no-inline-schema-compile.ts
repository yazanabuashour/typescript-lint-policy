...
function isStaticSchemaReference(
...
function isNestedStaticSchemaCall(
  node: ESTree.Node | null | undefined,
): boolean {
  const expression = unwrapExpression(node)
...
export const noInlineSchemaCompileRule = defineRule({
  meta: {
    ...
    docs: {
      description:
        "Disallow rebuilding Effect Schema decoder and encoder compilers inside function bodies.",
    },
  },
  createOnce(context) {
    let functionDepth = 0
