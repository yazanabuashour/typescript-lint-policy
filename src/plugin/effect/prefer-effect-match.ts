...
import { preferEffectMatchRule as upstreamRule } from "../../vendor/anti-slop/src/effect/rules/prefer-effect-match.ts"
import { tagMemberFromComparison } from "../../vendor/anti-slop/src/effect/shared/tagged-values.ts"
...
export const preferEffectMatchRule = defineRule({
  ...
  createOnce(context) {
    ...
    if (checkConditional === undefined) {
      throw new Error(
        "Upstream prefer-effect-match must visit conditional expressions",
      )
    }
