...
import { noManualEffectErrorTagRule as upstreamRule } from "../../vendor/anti-slop/src/effect/rules/no-manual-effect-error-tag.ts"
import {
  isReasonTagMember,
  isTagMember,
  ...
  createOnce(context) {
    return {
      BinaryExpression(node) {
        ...
        if (tagMember === undefined || !isInsideBroadEffectHandler(node)) return
